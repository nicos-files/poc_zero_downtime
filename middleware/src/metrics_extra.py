...
HEARTBEAT_TABLE_PG  = 'public."heartbeat"'     # en PG
...
TABLES: List[Tuple[str, str]] = [
    ("dbo", "customers"),
    ("dbo", "orders"),
    # ("dbo", "heartbeat"),  # si querés contarla también
]
...
def _heartbeat_pg_iso() -> str | None:
    """
    Lee el ts_utc que ahora viene como BIGINT epoch ms en Postgres
    y lo devuelve como string ISO-8601 UTC (2025-12-04T19:10:11.123Z).
    """
    sql = f"""
    SELECT to_char(
             to_timestamp("ts_utc" / 1000.0) AT TIME ZONE 'UTC',
             'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'
           ) AS ts
    FROM {HEARTBEAT_TABLE_PG}
    WHERE "ID" = 1
    """
...
def _loop():
    while True:
        ...
        for schema, table in TABLES:
            try:
                # MSSQL: [dbo].[customers], [dbo].[orders]
                fq_src = f"[{schema}].[{table}]"

                # Postgres: "public"."customers", "public"."orders"
                fq_dst = f'"public"."{table.lower()}"'
